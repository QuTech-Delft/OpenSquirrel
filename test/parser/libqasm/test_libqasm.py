...
from opensquirrel.ir import BlochSphereRotation, ControlledGate, Float, Gate, Int, Qubit, Measure, Bit
...
@pytest.mark.parametrize(
    ("circuit_string", "expected_result"),
    [
        ("version 3.0; qubit q; X q", [BlochSphereRotation(qubit=0, axis=(1, 0, 0), angle=math.pi, phase=math.pi / 2)]),
        ("version 3.0; qubit q; Rx(pi) q", [BlochSphereRotation(qubit=0, axis=(1, 0, 0), angle=math.pi, phase=0)]),
        (
            "version 3.0; qubit[2] q; CNOT q[0], q[1]",
            [ControlledGate(0, BlochSphereRotation(qubit=1, axis=(1, 0, 0), angle=math.pi, phase=math.pi / 2))],
        ),
    ],
    ids=["bloch_sphere_rotation", "parametrized_bloch_sphere_rotation", "controlled_gate"],
)
