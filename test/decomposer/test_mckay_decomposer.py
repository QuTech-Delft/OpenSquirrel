...
@pytest.mark.parametrize(
    ...
    [(CNOT()(0, 1), [CNOT()(0, 1)]), (CR(2.123)(2, 3), [CR(2.123)(2, 3)])],
)
...
def test_y(decomposer: McKayDecomposer) -> None:
    ...
    decomposed_gate = decomposer.decompose(gate)
    check_gate_replacement(gate, decomposed_gate)
    assert decomposed_gate == [Rz(math.pi)(0), X90()(0), X90()(0)]
...
def test_z(decomposer: McKayDecomposer) -> None:
    ...
    decomposed_gate = decomposer.decompose(gate)
    check_gate_replacement(gate, decomposed_gate)
    assert decomposed_gate == [Rz(math.pi)(0)]
...
def test_rz(decomposer: McKayDecomposer) -> None:
    ...
    decomposed_gate = decomposer.decompose(gate)
    check_gate_replacement(gate, decomposed_gate)
    assert decomposed_gate == [Rz(math.pi / 2)(0)]
...
def test_hadamard(decomposer: McKayDecomposer) -> None:
    ...
    decomposed_gate = decomposer.decompose(gate)
    check_gate_replacement(gate, decomposed_gate)
    assert decomposed_gate == [Rz(math.pi / 2)(0), X90()(0), Rz(math.pi / 2)(0)]
...
def test_arbitrary(decomposer: McKayDecomposer) -> None:
    ...
    assert decomposed_arbitrary_operation == [
        Rz(0.018644578210707863)(0),
        X90()(0),
        Rz(2.520651583905213)(0),
        X90()(0),
        Rz(2.2329420137988887)(0),
    ]
