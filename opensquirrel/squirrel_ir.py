...
class BlochSphereRotation(Gate):
    ...
    def __init__(self, qubit: Qubit, axis: Tuple[float, float, float], angle: float, phase: float = 0):
        ...
        self.axis = self._normalize_axis(np.array(axis).astype(np.float64))
    ...
    def __repr__(self):
        return f"BlochSphereRotation(q[{self.qubit.index}], axis={self.axis}, angle={self.angle}, phase={self.phase})"
...
class MatrixGate(Gate):
    ...
    def __eq__(self, other):
        # Should we allow a global phase difference here?
        return np.allclose(self.matrix, other.matrix)

    def __repr__(self):
        return f"MatrixGate(qubits={list(map(lambda q: q.index, self.operands))}, matrix={self.matrix})"
...
class ControlledGate(Gate):
    ...
    def __init__(self, control: Qubit, target_gate: Gate):
    ...
    def __repr__(self):
        return f"ControlledGate(control_qubit={self.control.index}, {self.target_gate})"
