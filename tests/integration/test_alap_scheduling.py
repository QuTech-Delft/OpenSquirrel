...
def _check_ref_schedulables(exported_schedule, expected_ref_schedulable_indices: list[int]) -> None:
    ref_schedulables = [schedulable_data['timing_constraints'][0]['ref_schedulable'] for schedulable_data in
                        list(exported_schedule.schedulables.values())]
    schedulable_index_map = {None: None, **{schedulable: index for index, schedulable in
                                            enumerate(list(exported_schedule.schedulables.keys()))}}
    for ref_schedulable, expected_ref_schedulable_index in zip(ref_schedulables, expected_ref_schedulable_indices):
