...
def test_route_indices_propagation(router4: ShortestPathRouter, circuit4: Circuit) -> None:
    ...
    for actual, expected in zip(actual_statements, expected_statements, strict=False):
        ...
        actual_indices = [q.index for q in actual.qubit_operands]  # type: ignore[attr-defined]
        expected_indices = [q.index for q in expected.qubit_operands]  # type: ignore[attr-defined]
