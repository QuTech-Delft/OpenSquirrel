...
In this scenario, all 2-qubit gates are executable on the hardware and no output is produced.
On the other hand, if there are indeed qubit connections that do not exist on the device, a `ValueError` will be raised.
...
The `NativeGateValidator` will raise a `ValueError` if there are gates on the circuit that are not present in the native
gate set of the quantum device.
...
validator = PrimitiveGateValidator(native_gate_set = native_gate_set)
